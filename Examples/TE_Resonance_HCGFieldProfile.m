%% TE_Resonance_HCGFieldProfile.m
...
DC=0.70;                    % [1] HCG duty cycle
tg=0.84180*period;          % [m] HCG thickness
...
lambda=(2.32947106:2e-12:2.329471064)*period;        % [m] incident wave wavelength
...
for idx=1:length(lambda)
    ...
    powRefl0th(idx)=(abs(fldRefl((1+end)/2)))^2;   % the 0th order power reflection coefficient
    powTrans0th(idx)=(abs(fldTrans((1+end)/2)))^2; % the 0th order power transmission coefficient
end

figure;
plot(lambda/period-2.329471062, powRefl0th);
xlabel('\lambda/\Lambda (offset by 2.329471062)');
...
lambda0=2.3294710618*period;              % [m] incident light wavelength in vacuum
tic;
[~, ~, H_all, E_all, incidentWaveAmp ] = solveFieldProfile( a, s, eBar, uBar, e1, u1, e2, u2, lambda0, tg, ord, isTM, x, z );
tElapsed=toc;
