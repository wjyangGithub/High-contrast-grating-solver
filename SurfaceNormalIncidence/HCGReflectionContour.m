...
% and incident light wavelength, with a plane wave incident at surface normal incident angle.
...
lambda=(0.5:0.02:4)*period;       % [m] incident wave wavelength
...
for idx2=1:length(lambda)
    [ka, ks, beta] = solveKaKsBeta(a, s, eBar, uBar, e2, u2, lambda(idx2), ord+1, isTM);
    [H, E, ~, gamma] = solveHE(ka, ks, a, s, eBar, uBar, e1, u1, e2, u2, a+s, lambda(idx2), ord+1, ord+1, isTM);
    for idx1=1:length(tg)
        [powRefl, powTrans, fldRefl, fldTrans, ~] = solveRT(H, E, a+s, lambda(idx2), e1, u1, beta, gamma, tg(idx1), ord+1, ord+1, isTM);
