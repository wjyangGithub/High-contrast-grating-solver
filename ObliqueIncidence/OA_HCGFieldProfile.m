...
% with a plane incident at an incident angle.
...
tic;
[~, ~, H_all, E_all, incidentWaveAmp ] = solveFieldProfile_OA( s, a, tg, eBar, uBar, e1, u1, e2, u2, lambda, theta, ord, isTM, x, z );
tElapsed=toc;
...
Z_E_field=[E_all./incidentWaveAmp(2) ...
    E_all./incidentWaveAmp(2)*exp(-1i*2*pi*n1/lambda*sin(theta)*period) ...
    E_all./incidentWaveAmp(2)*exp(-2*1i*2*pi*n1/lambda*sin(theta)*period)];
...
Z_H_field=[H_all./incidentWaveAmp(1) ...
    H_all./incidentWaveAmp(1)*exp(-1i*2*pi*n1/lambda*sin(theta)*period) ...
    H_all./incidentWaveAmp(1)*exp(-2*1i*2*pi*n1/lambda*sin(theta)*period)];
